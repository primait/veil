...
pub fn derive_mask(e: syn::DataEnum, attrs: Vec<syn::Attribute>, name_ident: syn::Ident) -> Result<TokenStream, syn::Error> {
        ...
        [Some(flags)] => {
            if !flags.all || !flags.variant {
                return Err(syn::Error::new(
                    attrs[0].span(),
                    "at least `#[mask(all, variant)]` is required here to mask all variant names",
                ));
            } else {
                Some(flags)
            }
        ...
        let mut flags = match FieldFlags::extract::<2>(&variant.attrs)? {
            ...
            [Some(flags), None] => {
                if flags.all && flags.variant {
                    ...
                    return Err(syn::Error::new(
                        attrs[0].span(),
                        "`#[mask(all, variant, ...)]` is invalid here, split into two separate attributes instead to apply masking options to the variant name or all fields respectively",
                    ));
                } else if flags.all {
                    // #[mask(all, ...)]
                    ...
                    return Err(syn::Error::new(
                        variant.span(),
                        "expected `#[mask(all, ...)]` or `#[mask(variant, ...)]`, or both as separate attributes",
                    ));
                }
            },
                ...
                for flags in [flags0, flags1] {
                    if flags.all && flags.variant {
                        ...
                        if variant_flags.variant_flags.is_some() {
                            return Err(syn::Error::new(
                                variant.span(),
                                "a `#[mask(variant, ...)]` is already present",
                            ));
                        }
    ...
    let variant_destructures = e.variants.iter().map(|variant| {
        match &variant.fields {
            ...
            syn::Fields::Unnamed(syn::FieldsUnnamed { unnamed, .. }) => {
                let args = (0..unnamed.len()).into_iter().map(|i| syn::Ident::new(&format!("arg{i}"), unnamed.span()));
                quote! {
                    ( #(#args),* )
                }
            },
    ...
    for (variant, flags) in e.variants.iter().zip(variant_flags.into_iter()) {
        // Variant name masking
        let variant_name = variant.ident.to_string();
        let variant_name = if let Some(flags) = &flags.variant_flags {
            quote! {
                ::veil::private::mask(&#variant_name, ::veil::private::MaskFlags {
                    debug_alternate,
                    is_option: false,
                    #flags
                })?
        ...
        variant_bodies.push(match &variant.fields {
            syn::Fields::Named(named) => FormatData::FieldsNamed(named).impl_debug(variant_name, flags.all_fields_flags, false)?,
            syn::Fields::Unnamed(unnamed) => FormatData::FieldsUnnamed(unnamed).impl_debug(variant_name, flags.all_fields_flags, false)?,
